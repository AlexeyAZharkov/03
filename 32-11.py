...
class RenderDigit:
    def __call__(self, inp_str, *args, **kwargs):
        if re.fullmatch(r'^-{0,1}\d+', inp_str):
            return int(inp_str)
...
class InputValues:
    ...
    def __call__(self, func):
        def wrapper(*args, **kwargs):
            return [self.__render(d) for d in func().split()]
        return wrapper
...
в другой тип данных. Чтобы реализовать такой декоратор в инициализаторе __init__() следует указать параметр render, 
